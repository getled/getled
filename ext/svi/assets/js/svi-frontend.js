...
WOOSVI.STARTS = function ( $, window, document, undefined ) {
	...
	var $is_variation = $form.find( '.variations select' ).length > 0;
	...
	//THUMBNAILS
	var $hide_thumbs = WOOSVIDATA.hide_thumbs == '1';
	var $woosvi_lens = WOOSVIDATA.lens == '1';
	var $woosvi_lightbox = WOOSVIDATA.lightbox == '1';
	var $prettyphoto_running = false;
	var $LoadLens_running = false;
	var $exist_thumbs = typeof WOOSVIDATA.gallery.thumbs !== 'undefined' && WOOSVIDATA.gallery.thumbs.length > 0;
	/*BROWSER SUPPORT*/
	var $browser = (
	...
	var $imagesLoaded = WOOSVIDATA.imagesloaded == '1';
	return {
		NAME: "Application initialize module", VERSION: WOOSVIDATA.jsversion,
		init: function () {
			if ( $browser ) {
				$woosvi_lens = false;
			}
